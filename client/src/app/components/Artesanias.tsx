  ...
  return (
    <>
      <section className="w-full grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] auto-rows-[440px] gap-4 mb-5">
        {pagination.artesanias[pagination.page] ? (
          pagination.artesanias[pagination.page].map((el: Artesania) => {
            return (
              <article
                key={el.idArtesania}
                className="bg-beige2 rounded-xl relative overflow-hidden flex flex-col items-center hover:scale-[1.03] transition duration-200"
              >
                <Image
                  src={
                    isValidUrl(el.imagen) || isValidRelativeUrl(el.imagen)
                      ? el.imagen
                      : "https://www.madeinnobsa.com/wp-content/uploads/2023/10/crop-0-0-400-400-0-LanaOveja-1.webp"
                  }
                  alt=""
                ...
                />
                <h3 className="font-black text-3xl leading-12 line-clamp-3 text-shadow-lg mx-3 text-center">
                  {el.nombre}
                </h3>
                <p className="font-bold text-xl leading-none line-clamp-3 text-center mx-3">
                  {el.descripcion}
                </p>
                <Link
                  href={`/update?id=${el.idArtesania}`}
                  className="absolute right-0 top-0 m-2 bg-beige2 p-2 rounded-full hover:scale-110 transition duration-300"
                >
                  <svg
                    className="fill-chocoBlack"
